...
namespace AltaMontanha.Models.Fachada
{
    public class MultimidiaFacade
    {
        ...
        //                                   };
        private static dynamic[] tamanhos = { 
			new { largura = 1024, altura = 0, caminho = "full" }
            ,new { largura = 300, altura = 0, caminho = "300" }
            //,new { largura = 145, altura = 95,  caminho = "145x95" }
            ,new { largura = 340, altura = 240, caminho = "340x240" }
		};
        private static dynamic[] tamanhosServico = { 
			new { largura = 300, altura = 0, caminho = "full" }
            ,new { largura = 220, altura = 220, caminho = "220" }
            ,new { largura = 280, altura = 280, caminho = "280" }
		};
        ...
        /// <param name="arquivo">arquivo da foto</param>
        /// <returns>Foto cadastrada com o código gerado</returns>
        public void SalvarFotoServico(HttpPostedFileBase file)
        {
            try
            {
                IFactoryDAO fabrica = FactoryFactoryDAO.GetFabrica();
                ...
                // Para manter a proporcao da imagem eh necessario verificar se a imagem eh horizontal ou vertical
                if (imagem.Size.Width > imagem.Size.Height)
                    Proporcao = (double)imagem.Size.Height / (double)imagem.Size.Width;
                else
                    Proporcao = (double)imagem.Size.Width / (double)imagem.Size.Height;

                foreach (dynamic tamanho in tamanhosServico)
                {
                    caminho = string.Format("{0}\\{1}\\{2}", HttpContext.Current.Server.MapPath("~/AppData/Foto/logo/servicos"), tamanho.caminho, file.FileName);

                    if (imagem.Size.Width > imagem.Size.Height)
                        this.SalvarImagem
                        (
                            this.RedimensionarImagem(file.InputStream, tamanho.largura, (int)(tamanho.largura * Proporcao)),
                            caminho
                        );
                    else
                        this.SalvarImagem
                        (
                            this.RedimensionarImagem(file.InputStream, (int)(tamanho.largura * Proporcao), tamanho.largura),
                            caminho
                        );
                }
            }
        ...
        /// <returns></returns>
        public Bitmap RedimensionarImagem(Stream stream, int altura, int largura)
        {
        ...
        private string GerarCaminhoImagem(HttpPostedFileBase file)
        {
            ...
            if (file.ContentLength > 0)
            {
                ...
                foreach (dynamic tamanho in tamanhos)
                {
                    ...
                    diretorio = string.Format(@"{0}\{1}", HttpContext.Current.Server.MapPath("~/AppData/Foto"), tamanho.caminho);
